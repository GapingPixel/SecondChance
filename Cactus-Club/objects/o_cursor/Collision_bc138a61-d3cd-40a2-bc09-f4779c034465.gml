...
if (!instance_exists(o_description)) {
	if !global.grace_pescatarian {
		...
		create_description("Grace is pescatarian", 1 o_inventory.x+450, o_inventory.y+28);
	}
	show_des = 2;
}
...
if (mouse_check_button_pressed(mb_left)) {	
	if !global.grace_pescatarian {
		if state.chat {
			...
			scr_text("Aren't you a vegetarian, Grace?",01view_xport[0]+125,view_yport[0]+250,CHEST_PROT);
